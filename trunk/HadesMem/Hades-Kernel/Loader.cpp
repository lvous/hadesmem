...
namespace Hades
{
    ...
    PVOID Unknown2)
  {
    ...
    {
      // Get trampoline
      typedef BOOL (WINAPI* tCreateProcessInternalW)(PVOID Unknown1, 
        LPCWSTR lpApplicationName, LPWSTR lpCommandLine, 
        LPSECURITY_ATTRIBUTES lpProcessAttributes, 
        LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, 
        DWORD dwCreationFlags, LPVOID lpEnvironment, LPCWSTR lpCurrentDirectory, 
      ...
      // Debug output
      std::wcout << "Loader::CreateProcessInternalW_Hook: Called." << std::endl;
      ...
      // Call original API
      BOOL RetVal = pCreateProcessW(Unknown1, lpApplicationName, lpCommandLine, 
        lpProcessAttributes, lpThreadAttributes, bInheritHandles, 
        (dwCreationFlags | CREATE_SUSPENDED), lpEnvironment, lpCurrentDirectory, 
        lpStartupInfo, lpProcessInformation, Unknown2);
    ...
    const std::wstring& Module)
  {
    ...
    try
    {
        ...
        IsWow64Process(ProcInfo->hProcess, &IsWoW64) && !IsWoW64)
      {
        ...
        std::wstring const ModulePath(Windows::GetSelfDirPath().file_string() + 
          L"\\" + Module);
