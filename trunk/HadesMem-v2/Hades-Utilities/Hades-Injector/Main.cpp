...
#if defined(_M_X64)
    std::wcout << "Hades-Injector AMD64 [Version " << VerNum << "]\n";
#elif defined(_M_IX86)
    std::wcout << "Hades-Injector IA32 [Version " << VerNum << "]\n";
#else
#error Unsupported platform!
    ...
    bool KeepOpen = false;
    // Path to module (Set by Boost.ProgramOptions)
    std::wstring ModulePath;
    // Export name (Set by Boost.ProgramOptions)
    ...
    // Inject DLL
    HMODULE ModRemote = MyInjector.InjectDll(ModulePath);
    std::wcout << "Module Base: " << ModRemote << "." << std::endl;

    // Call remote export
    if (!ExportName.empty())
    {
      DWORD_PTR ExportRet = MyInjector.CallExport(ModulePath, ModRemote, 
        ExportName);
